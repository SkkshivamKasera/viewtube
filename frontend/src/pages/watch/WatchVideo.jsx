  ...
  return (
    <div className='w-full lg:flex lg:px-10 lg:py-2'>
      <div className='w-full lg:w-3/4'>
        ...
        <div className='px-2'>
          <p className='text-justify text-gray-950 font-semibold'>
            {video && window.innerWidth > 800 ? (video.title.length >= 100 ? video.title.slice(0, 281) + ' ...' : video.title) : video && video.title.length >= 100 ? video.title.slice(0, 95) + ' ...' : video.title}
          </p>
          <div className='lg:flex lg:items-center lg:justify-between'>
            <div className='flex items-center justify-between w-full lg:w-[400px]'>
              <div className='flex items-center'>
                <Link to={`/channel/user/${video && video.user && video.user._id}`} className='flex items-center'>
                  <div className='w-20 h-20 rounded-full p-2'>
                    <img src={video && video.user && video.user.avatar} alt={`${video && video.user && video.user._id}`} className='w-full h-full rounded-full' />
                  </div>
                  <div>
                    <p className='capitalize font-bold'>{video && video.user && video.user.channelName}</p>
                    <p className='capitalize'>{video && video.user && formatValue(video.user.subscribers)} subscribers</p>
                  </div>
                </Link>
              </div>
              <div className='ml-4'>
            ...
            <div className='flex items-center justify-between w-full lg:w-auto lg:ml-10 relative'>
              <div className='flex items-center px-4 py-2 rounded-3xl bg-gray-200 lg:mr-10 hover:bg-gray-600 hover:text-white font-semibold cursor-pointer' onClick={like}>
                ...
                <span className='ml-2'>{video && video.likes && formatValue(video.likes.length)}</span>
              </div>
          ...
          <div className='w-full bg-gray-200 mt-2 rounded-3xl'>
            <p className='flex items-center p-4'>
              <span className='font-bold capitalize'>{video && formatValue(video.views)} views</span>
              <span className='ml-3 capitalize font-bold'>{video && timeDifference(video.createdAt)}</span>
            </p>
            <p className='px-4 text-justify text-xs lg:text-[16px] leading-5'>
              {video && window.innerWidth > 800 ? (!showFullDesc ? video.desc.slice(0, 270) : video.desc) : video && !showFullDesc ? video.desc.slice(0, 113) : video.desc}
              {(video && video.desc.length > 230) && !showFullDesc ? (
        ...
        <div className='hidden lg:block px-2'>
          <h1 className='font-bold uppercase text-3xl lg:text-4xl p-2'>{video && video.comments && formatValue(video.comments.length)} Comments</h1>
          <div>
            <CommentInput videoID={video && video._id} commentTo={'VIDEO'} />
          </div>
        ...
        <div className='block px-2 bg-gray-200 rounded-2xl mx-2 mt-2 lg:hidden'>
          <Link to={"/comments"}>
            <p className='font-bold text-xl px-2 uppercase py-2'>
              {
                video && video.comments ? formatValue(video.comments.length) : 0
              }
              {" "}comments
            </p>
      ...
      <div className='px-2 mt-2 w-full'>
        {
          video && video.user && video.user.videos && video.user.videos.length > 0 && (
